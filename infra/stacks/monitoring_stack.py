...
class MonitoringStack(Stack):
    ...
    def _add_worker_section(self, db: cloudwatch.Dashboard) -> None:
        ...
        cpu_metrics = [
            self._m(
                ns,
                "CPUUtilization",
                {"ClusterName": self.cluster_name, "ServiceName": svc},
                "Average",
                p1,
            )
    ...
    def _add_pipeline_section(self, db: cloudwatch.Dashboard) -> None:
        ...
        queue_depths = [
            self._m(
                "AWS/SQS", "ApproximateNumberOfMessagesVisible", {"QueueName": q}, "Sum"
            )
            for q in queues
        ]
        dlq_depths = [
            self._m(
                "AWS/SQS", "ApproximateNumberOfMessagesVisible", {"QueueName": d}, "Sum"
            )
            for d in dlqs
        ]
    ...
    def _add_aurora_section(self, db: cloudwatch.Dashboard) -> None:
        ...
        db.add_widgets(
            self._graph(
                "Aurora ACU",
                [self._m(ns, "ServerlessDatabaseCapacity", dims, "Average")],
            ),
            self._graph(
                "DB Connections", [self._m(ns, "DatabaseConnections", dims, "Average")]
            ),
            self._graph(
                "DB Throughput",
                [
                    self._m(ns, "CommitThroughput", dims),
                    self._m(ns, "SelectThroughput", dims),
                ],
            ),
            self._graph(
                "DB Storage", [self._m(ns, "VolumeBytesUsed", dims, "Average")]
            ),
        )
    ...
    def _add_bedrock_section(self, db: cloudwatch.Dashboard) -> None:
        ...
        db.add_widgets(
            self._graph(
                ...
                [
                    self._m(ns, "Invocations", dims),
                    self._m(ns, "InvocationLatency", dims, "Average"),
    ...
    def _add_batch_inference_section(self, db: cloudwatch.Dashboard) -> None:
        ...
        db.add_widgets(
            self._graph(
                "Batcher Invocations",
                [
                    self._m(ns, "Invocations", b_dims),
                    self._m(ns, "Errors", b_dims),
                ],
            ),
            self._graph(
                "Batcher Duration",
                [
                    self._m(ns, "Duration", b_dims, "Average"),
                    self._m(ns, "Duration", b_dims, "p99"),
                ],
            ),
            self._graph(
                "Result Processor Invocations",
                [
                    self._m(ns, "Invocations", r_dims),
                    self._m(ns, "Errors", r_dims),
                ],
            ),
            self._graph(
                "Result Processor Duration",
                [
                    self._m(ns, "Duration", r_dims, "Average"),
                    self._m(ns, "Duration", r_dims, "p99"),
                ],
            ),
        )
    ...
    def _add_autoscaling_section(self, db: cloudwatch.Dashboard) -> None:
        ...
        db.add_widgets(
            self._scaling_widget(
                "Worker Scaling", self.worker_service_name, self.queue_name
            ),
            self._scaling_widget(
                "Validator Scaling",
                self.validator_service_name,
    ...
    def _create_alarms(self) -> None:
        ...
        self._alarm(
            ...
            self._m("AWS/Lambda", "Errors", api_dims),
            5,
            2,
            GTE,
        ...
        self._alarm(
            ...
            self._m("AWS/Lambda", "Throttles", api_dims),
            1,
            2,
            GTE,
        ...
        self._alarm(
            ...
            self._m(
                "AWS/DynamoDB", "ThrottledRequests", {"TableName": self.jobs_table_name}
            ),
            1,
            1,
            GTE,
        ...
        self._alarm(
            ...
            self._m(
                "AWS/Bedrock", "InvocationThrottles", {"ModelId": self.bedrock_model_id}
            ),
            5,
            2,
            GT,
        ...
        for fn_name, cid_prefix, label in [
            ...
            (
                self.result_processor_function_name,
            ...
            self._alarm(
                ...
                self._m("AWS/Lambda", "Errors", fn_dims),
                5,
                1,
                GTE,
            ...
            self._alarm(
                ...
                self._m("AWS/Lambda", "Throttles", fn_dims),
                1,
                1,
                GTE,
        ...
        for label, slug, tbl in [
            ("ContentIndex", "content-index", self.content_index_table_name),
