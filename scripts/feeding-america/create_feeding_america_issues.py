...
def create_github_issue(issue_data: Dict[str, any], dry_run: bool = True) -> bool:
    ...
    labels = [
        label for label in issue_data.get("labels", []) if label in existing_labels
    ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create GitHub issues for Feeding America food banks"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--only-vivery",
        action="store_true",
        help="Only create issues for Vivery candidates",
    )
    parser.add_argument(
        "--exclude-vivery", action="store_true", help="Exclude Vivery candidates"
    )
    parser.add_argument(
        "--state", help="Only create issues for a specific state (e.g., NY)"
    )
    parser.add_argument(
        "--delay",
        type=int,
        default=2,
        help="Delay between issues in seconds (default: 2)",
    )
    parser.add_argument(
    ...
    print(
        f"Will create: {len(issues_to_create)} issues (starting from index {args.start})"
    )
