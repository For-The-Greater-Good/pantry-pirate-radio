...
-- `id` and the Merkle leaf input). `preimage_canonical` stores the EXACT JCS
-- bytes that were hashed/signed, so the Merkle leaf is re-derived verbatim (a
-- JSONB round-trip normalizes extreme-magnitude numbers and would break proofs);
-- `object_canonical` retains the full envelope JSONB for queryability only.
