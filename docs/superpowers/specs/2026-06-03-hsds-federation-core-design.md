...
### 8.5a Canonical HSDS-FX conformance suite (owner-confirmed 2026-06-06: BOTH layers, in-repo first)

The conformance suite is the artifact that turns "our implementation" into "a standard anyone can implement against," and is the centerpiece brought to the Open Referral TC. It is **two layers**:

- **Level 1 — wire conformance (portable, offline, the RFC core).** Static canonical vectors: given fixed inputs, an implementation's bytes MUST match — envelope `id` (content address) + `proof`, JCS canonicalization, RFC-6962 inclusion/consistency proofs, the C2SP checkpoint note, the `federation_id` grammar, and the `/export`+`/checkpoint`+`/state.txt`+`/history` response shapes — plus negative/reject cases (malformed signature, tampered leaf, non-canonical base64, below-window). A foreign implementer runs these with no running PPR node. **Anti-self-grading (constitution v1.7.x):** every *primitive* layer is already anchored to an upstream external suite (JCS RFC-8785, RFC-6962 transparency-dev, C2SP Go `sumdb/note`, RFC-9421, did:key, RFC-7386); Level 1's *new* canonical contribution is the **HSDS-FX composition** (envelope assembly, grammar, wire shapes), which is cross-validated by the two-node interop loop and, eventually, a non-PPR implementation.
- **Level 2 — behavioral interop (needs a running node).** The live interaction semantics: pull/ingest, `Update`/`Announce`/`Delete` authority, corroboration + origin-dedup, tombstone `redirectTo`. Driven against the in-repo reference second node (§15 golden journeys); a higher tier a node certifies only if it also ingests.

**Sequencing (owner-confirmed): pull forward now, in-repo.** Build the fixtures corpus + a conformance runner under `tests/` first (it doubles as PPR's own CI conformance gate immediately), then extract to the standalone neutrally-named HSDS-FX artifact once it stabilizes — "build the running reference first; extract in parallel" (§8.5). This unblocks the RFC artifact without waiting for P2.

### 8.6 Packaging & adoption (STRETCH GOALS — make adoption one line, not a re-implementation)

Driver: a partner should adopt HSDS-FX with **a one-line install, never a from-scratch protocol re-implementation** — and, ideally, by reusing our open-source app outright. The adoption ladder, lowest-effort first:

1. **Zero-install web** *(designed, §8.5):* the hosted **Federation Readiness Checker** (paste your `/export` URL → tier report) and the **copy-paste static-feed generator** (Tier-1 in an afternoon).
2. **Published packages** *(STRETCH — the new documented goal):*
   - **Canonical vectors package** — the conformance suite as language-agnostic data (versioned), published so any implementation in any language pulls the same fixtures. (npm + PyPI at minimum; the raw JSON is the source of truth.)
   - **Conformance runner / validator** — `pip install hsdsfx` (+ npm) exposing `hsdsfx verify <export-url>` → a tier report, running Level 1 (and Level 2 against a live node). The Readiness Checker is this package behind a URL.
   - **Reference client / SDK** — a thin library to *produce and verify* envelopes, proofs, and checkpoints (Ed25519 + JCS + RFC-6962 + C2SP), so an adopter never re-implements the crypto. Python first (extracted from `app/federation/` pure modules), then a second language to prove the spec is implementable cross-stack.
3. **Clone-able reference node** *(STRETCH):* a published container image / template repo (`docker run hsdsfx-node` or a one-command `./bouy`-style bootstrap) so a partner stands up a *full federating node by reusing our app* — the path the first real partner (a check-in tool that would reuse our OSS rather than re-implement) explicitly wants. The §15 in-repo reference second node is the seed for this image.

**Status:** stretch goals — not gating for any phase, tracked so they are not forgotten. Natural home: alongside the HSDS-FX extraction (P1 PR-D Task 11) and the P2 reference-node work, maturing toward the RFC. Each published package MUST itself ship its conformance vectors + a green conformance run (dog-food: the SDK passes the suite it ships).
...
| **Adoption packaging** *(STRETCH — §8.6, not phase-gating)* | Published **canonical-vectors** + **conformance-runner/validator** (`hsdsfx verify <url>`) + **reference client/SDK** packages (PyPI + npm) + a **clone-able reference-node image**, so adoption is a one-line install (or app-reuse), not a re-implementation. Each package dog-foods the conformance suite it ships. Matures alongside HSDS-FX extraction (PR-D Task 11) + the P2 reference node, toward the RFC. | adopters |
