...
- **P1 — Verifiable publish: ✅ COMPLETE — #522 CLOSED (2026-06-08).** 4-PR split + PR-D's conformance/closing slices, all merged to `main`. **PR-A (decomposition), PR-B (RED-tier substrate: `FederationLog` + dense-sequence append + RFC-6962 Merkle frontier + C2SP signed checkpoints + inclusion/consistency proofs + aggregate + JCS vectors), PR-C (hooks at the reconciler `location_commit`/dedup-script/submarine commit sites via `app/federation/publish.py` + kill-switch + `/api/v1/federation/{export,state.txt,checkpoint,history}`, #563)** merged. **PR-D shipped across 11 machine-reviewed PRs, every one RED-tier-Gauntlet-gated:** the **HSDS-FX conformance suite** (#567 envelope/checkpoint/export+Level-2; #568 federation_id grammar; #569 JCS anchor + honesty hardening; #570 activity verbs/Tombstone; #571 RFC-3986 mechanic anchor; #572 RFC-6962 consistency-proof anchor — 10 areas, 19 externally-anchored vectors); the **§15 two-node golden journey** (#573 — `identity.public_key_from_multibase` decoder; B discovers A's key from `/.well-known/did.json`, pulls `/export@N`, cross-verifies checkpoint + every envelope sig + RFC-6962 inclusion + consistency); **cold-start `_since=0` raw-table parity** (#574 — byte-equal `build_location_aggregate` vs the served object, anti-flatten teeth); **archive tiering + dual-env retention-prune + Principle-XIV alarm** (#575 — `app/federation/retention.py` archive-then-trim with `leaf_data` read-back, `./bouy federation prune` + EventBridge Lambda, dedicated no-lifecycle S3 bucket). **All P1 acceptance criteria met** (the §-below acceptance line). The Gauntlets caught + fixed real defects pre-merge (a foolable honesty-trace, a non-idempotent normalizer, a fabricated RFC citation, a two-node equivocation gap, 3 archive data-safety hardenings).
  - **Two documented P1 scope DEFERRALS (not regressions — beyond the acceptance bar, carried forward):** (a) the **hosted Readiness Checker (#565) + static-feed generator** named in the P1 roadmap row did NOT ship as standalone surfaces; the portable `verify_level2` runner (`tests/test_federation/conformance/runner.py`) IS the readiness-check mechanism (point it at a live node/URL) — a hosted service + a static-feed generator are a follow-on. (b) **Cold-start snapshot SERVING** (Task 8's "served from the HAARRRvest S3/SQLite snapshot" — the bulk fresh-peer bootstrap artifact): only the byte-equal PARITY TEST shipped; below-floor `/export` 410s with an archive pointer, and the archive WRITE + internal leaf read-back (which keep proofs valid forever) shipped — the bulk snapshot artifact + below-floor SERVING is the documented follow-on. (c) **Clarification:** the scoping had treated archive leaf read-back as deferrable; the code proved it REQUIRED (the checkpoint root@N recomputes from all N leaves, so trimming without read-back breaks every proof) — so it shipped in #575, correctly.

- **Scenario → phase map (owner ask 2026-06-07, "run through all the scenarios, swing big"):** the two-node ambition splits across two phases and must NOT be collapsed. **"Raw sync"** (publish→discover→pull→**verify** between two distinct instances, no writes) = **P1 — ✅ SHIPPED** (#573 §15 golden journey). **"Different datasets federating, with everything that entails"** (pull **ingest** as `source_type='federated_node'`, the §117 verify-before-enqueue order, corroboration + §12.1 origin-dedup, Update/Announce/Delete authority via `activities.py:validate_activity`, `merge_location` + HUMAN_VERIFIED guard, Tombstone `redirectTo` re-point, **bidirectional**) = **P2 Pull ingest** (roadmapped below, tasks 0–15). **P2 acceptance = two PPR instances (or PPR + the in-repo reference second node) carrying DIFFERENT datasets, ingesting + corroborating each other end-to-end** — NOT a "live Feeding America feed." ⚠️ **CORRECTION (owner, 2026-06-08): there is NO Feeding America HSDS feed/federation instance.** PPR *scrapes* FA food-bank sites to build HSDS data; FA publishes no consumable HSDS feed to federate-ingest from. The prior "live FA feed end-to-end" acceptance (and the §6.6a "adoption spearhead" framing) is INVALID and is replaced everywhere by the two-instance/PPR-to-PPR loop; a foreign/non-PPR HSDS publisher is P7 and none exists yet. The activity-verb validator (P1 #570) and the Level-2 runner (P1 #567) are the load-bearing inputs P2 consumes. The "No P2 code before P1 #522 closes" gate is now LIFTED.

- **🎯 P2 DRIVER + headline acceptance (owner, 2026-06-08) — LIVE multi-node federation:** an **unnamed partner organization will integrate once PPR demonstrates two of its OWN nodes federating LIVE.** The partner has **authority over a dataset complementary to PPR's** — so once they join, they ARE the real "different dataset" second node (and the authority/corroboration machinery below is what safely merges their authoritative records with PPR's). This makes **live multi-node federation the headline P2 deliverable**, not a stretch goal: two **separately-running PPR nodes** — each its own deployment, DB, `did:web` identity, and dataset — performing **pull → verify (checkpoint + envelope sig + inclusion + consistency) → ingest as `federated_node` → corroborate** over **real HTTP**, bidirectionally. This is a genuine step beyond P1 #573 (which was two instances of the same code **sequenced over one `federation_log` table** — honest integration evidence + the partner-reuse seed, but NOT separately-running nodes). **P2 acceptance therefore = two live PPR nodes with different datasets ingesting + corroborating each other end-to-end over the network** (the demonstration that unlocks the partner). A foreign/non-PPR implementation remains P7; the partner reuses PPR's OSS app, so two-PPR-nodes-live IS the integration they need to see.
...
| **P1 Verifiable publish ✅ DONE (#522 closed 2026-06-08)** | The design-§6.2 substrate: signed content-addressed objects, Merkle log, **signed checkpoints**, inclusion/consistency proofs; `/export`+`state.txt`+`/checkpoint`+`history`; hooks (job_processor matched/new-Location, **dedup scripts** `Delete`+`redirectTo`, submarine); **kill switch**; cold-start **verifiable snapshot** (raw tables, parity test); **archive tiering** (never destroy); **HSDS-FX spec extraction + fixtures/conformance suite + Readiness Checker + static-feed generator + in-repo reference second node + golden journey test**; Task 0 = `job_processor.py` decomposition (Principle IX, resolved: decompose). PPR is *verifiably readable*. | `app/federation/log.py`, `app/database/models.py`, migration, `app/reconciler/job_processor.py` (decomposed), `scripts/dedupe_*.py`, `app/reconciler/submarine_location_handler.py`, `app/api/v1/federation/`, `fixtures/federation/`, spec repo/artifact, `infra/` (prune/archive Lambda + alarm) | golden P1 journey: concurrent-append→pull→proof-verify→parity→archive boundary; kill-switch byte-identical test; fixtures validate; checkpoint consistency detects a rewritten log | none |
| **P2 Pull ingest** | Thin consumable enqueuer; consumer (PPR peers **with proof-verify before enqueue**; plain-HSDS snapshot-diff §6.6a as a general capability); the §12.3 corrections + **§12.1 origin-dedup + CvRDT order-shuffle property test**; un-corroborated gating + **§11.6a equity caveat**; per-peer budget; injection hardening; shared idempotency; VALIDATOR_ENABLED routing; **LIVE two-PPR-node federation (separate deployments/DBs/DIDs/datasets) over real HTTP**; Task 0 = IX gate for `merge_strategy.py`/`location_creator.py`; P2 observability (pull-consumer Lambda + ingest SQS + DLQ + budget alarm). **Demonstrates two live PPR nodes federating — the partner-integration gate.** | `app/federation/{enqueue,ingest}.py`, `app/reconciler/{merge_strategy,location_creator,job_processor}.py`, `app/llm/...`, `app/database/models.py` (peer+cursor via `CursorStore` protocol), `infra/` | **acceptance: two LIVE PPR nodes with DIFFERENT datasets pull + verify + ingest + corroborate each other end-to-end over HTTP** (unlocks the unnamed partner, who then joins as the real complementary-authority node); golden P2 journey (origin-dedup 100-announces→1; lone-peer gated; equity caveat served); enqueued job consumable by the aligner | a second running PPR node (we stand up both) — **NOT** an FA feed (no FA HSDS instance exists) |
...
**P2 acceptance:** **two LIVE PPR nodes — separate deployments, DBs, `did:web` DIDs, and DIFFERENT datasets — pull + verify + ingest + corroborate each other end-to-end over real HTTP** (the demonstration that unlocks the unnamed complementary-authority partner; ⚠️ *amended 2026-06-08 — the prior "live Feeding America HSDS feed" target is invalid, no FA HSDS instance exists*); the golden P2 journey passes (publish→`federated_node`→origin-dedup 100-announces→1 and 3-relays-of-one-origin→1→lone-peer gated→**equity caveat served where §11.6a applies**); the CvRDT order-shuffle property test passes; the enqueued job is consumable by the aligner; budget + injection + idempotency + validator-off-scoring + anomaly-demote tests pass; the pull-consumer Lambda + ingest SQS carry their XIV alarms + infra tests.
