...
**RESOLVED 2026-06-10 (owner):** **D1 → shrink-only ratchet now, HARD full-pass flip at Z3.** **D2 → one-release breaking switch to the official Page envelope** (no dual-emit; confirm HAARRRvest/Datasette/partner consumers before the G3 release — flag any breakage). **D3 → serve genuine HSDS `metadata[]`** change-records derived from `record_version`/`change_audit` (the flagship move; lands in T2, with G4 stopping the colliding wrapper emission). **D9 → INCLUDE the org graph in the demo's signed bytes** — O1/O2/X2 are demo blockers (no slip to partner-gate); the demo-blocker set is the full 16+ incl. the org embed. Still open: D4–D8, D10 (decided at their slice).

- **D1 — Conformance gate hardness:** ✅ RESOLVED — shrink-only ratchet now, HARD full-pass at Z3.
- **D2 — Page envelope cutover:** ✅ RESOLVED — one-release breaking switch (G3). POST list variants: defer unless a consumer needs them. ⚠️ G3 must verify no critical consumer (HAARRRvest export, Datasette, any partner) breaks on the `count/data`→`total_items/contents` change before release.
- **D3 — `metadata` strategy:** ✅ RESOLVED — serve genuine HSDS `metadata[]` from `record_version`/`change_audit` (T2); G4 stops emitting the colliding `MetadataResponse` wrapper.
...
- **D9 — Demo wire scope:** ✅ RESOLVED — INCLUDE the org graph (O1/O2/X2) in the demo's signed bytes; they are demo blockers, not partner-gate.
