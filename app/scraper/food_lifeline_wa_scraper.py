...
class FoodLifelineWAScraper(ScraperJob):
    ...
    def __init__(self, scraper_id: str = "food_lifeline_wa", test_mode: bool = False) -> None:
        ...
        self.geocoder = GeocoderUtils(
            default_coordinates={
                ...
                "WA": (47.400902, -121.490494),  # Food Lifeline region
    ...
    def parse_html(self, html: str) -> List[Dict[str, Any]]:
        ...
        for popup in location_popups:
            try:
                ...
                # Find email
                email_link = popup.find('a', href=lambda x: x and x.startswith('mailto:'))
                email = email_link['href'].replace('mailto:', '') if email_link else ""
                
                # Find website
                website_link = popup.find('a', string=lambda x: x and 'Visit Site' in x)
                website = website_link['href'] if website_link and website_link.get('href') else ""
    ...
    async def scrape(self) -> str:
        ...
        for location in locations:
            ...
            location_id = location.get('id') or f"{location.get('name', '')}_{location.get('address', '')}"
        ...
        for location in unique_locations:
            ...
            if not (location.get("latitude") and location.get("longitude")):
                # Try full address first, then street address
                address_to_geocode = location.get("full_address") or location.get("address")
                if address_to_geocode:
                    try:
                        ...
                        logger.warning(f"Geocoding failed for {address_to_geocode}: {e}")
            ...
            # Add metadata
            location["source"] = "food_lifeline_wa"
            location["food_bank"] = "Food Lifeline"
        ...
        print(f"\n{'='*60}")
        print(f"SCRAPER SUMMARY: Food Lifeline")
        print(f"{'='*60}")
