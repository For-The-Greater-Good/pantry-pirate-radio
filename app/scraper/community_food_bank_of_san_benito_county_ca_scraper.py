...
class CommunityFoodBankOfSanBenitoCountyCaScraper(ScraperJob):
    ...
    def __init__(self, scraper_id: str = "community_food_bank_of_san_benito_county_ca", test_mode: bool = False) -> None:
    ...
    def parse_html(self, html: str) -> List[Dict[str, Any]]:
        ...
        while i < len(lines):
            ...
            if in_mobile_section:
                ...
                if line.endswith(':') and line.rstrip(':') in ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday']:
                    current_day = line.rstrip(':')
                    i += 1
                    continue
                    
                # Parse mobile pantry locations (format: "Location – Time")
                if current_day and '–' in line and ('a.m.' in line or 'p.m.' in line) and not line.startswith('Time:'):
                    parts = line.split('–', 1)
                    if len(parts) == 2:
            ...
            # Static locations section parsing
                ...
                if line in ['Aromas', 'Hollister – Marketplace at Community Food Bank (Drive-Thru available)', 'San Juan Bautista', 'Tres Pinos']:
                    location_data = {"name": line, "services": ["food pantry", "pre-packed bags"]}
                    ...
                    while j < len(lines) and j < i + 5:  # Look ahead up to 5 lines
                        ...
                        if detail_line.startswith('Location:'):
                            location_data["address"] = detail_line.replace('Location:', '').strip()
                        elif detail_line.startswith('Time:'):
                            location_data["hours"] = detail_line.replace('Time:', '').strip()
                        elif 'Drive-Thru available' in detail_line:
                            location_data["services"].append("drive-thru")
                            location_data["notes"] = detail_line
                        elif any(day in detail_line for day in ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']) and not detail_line.startswith('Time:'):
