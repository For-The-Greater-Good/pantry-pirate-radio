...
class FoodBankOfContraCostaAndSolanoCAScraper(ScraperJob):
    ...
    def __init__(
        self,
        scraper_id: str = "food_bank_of_contra_costa_and_solano_ca",
        ...
        self.geocoder = GeocoderUtils(
            default_coordinates={
                ...
                "CA": (
                    36.116203,
                    -119.681564,
                ),  # Food Bank of Contra Costa and Solano region
    ...
    async def fetch_locations_from_export(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Fetch export data
            params = {"locations": ",".join(location_ids), "lang": "en-US"}
    ...
    def parse_location_div(self, div, city_name: str) -> Dict[str, Any]:
        ...
        address_elem = div.find(
            text=re.compile(
                r"\d+.*\b(St|Street|Ave|Avenue|Rd|Road|Blvd|Boulevard|Dr|Drive|Way|Ct|Court)\b",
                re.I,
            )
        )
        if address_elem:
    ...
    async def scrape(self) -> str:
        ...
        if not locations:
            ...
            for i, (city_name, city_url) in enumerate(city_links):
                ...
                try:
                    response = requests.get(
                        city_url, headers=get_scraper_headers(), timeout=self.timeout
                    )
                    response.raise_for_status()
        ...
        print(f"\n{'='*60}")
        print("SCRAPER SUMMARY: Food Bank of Contra Costa and Solano")
        print(f"{'='*60}")
