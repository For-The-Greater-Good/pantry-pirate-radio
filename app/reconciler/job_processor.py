...
class JobProcessor:
    ...
    def process_job_result(self, job_result: JobResult) -> dict[str, Any]:
        ...
        try:
            ...
            # Process locations. The per-location commit logic (preprocessing,
            # match resolution, and the matched-update / new-create commit
            # branches) lives in LocationCommitHandler (Principle IX). The
            # handler holds location_ids by reference, so new-location ids it
            # records are visible to the service/phone/schedule sections below.
            if "location" in data:
                location_commit_handler = LocationCommitHandler(
                    self.db,
                    ...
                    location_key_fn=self._location_key,
                    transform_schedule_fn=self._transform_schedule,
                )
                for location in data["location"]:
                    location_commit_handler.process_location(location, org_id)
