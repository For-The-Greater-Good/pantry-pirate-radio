...
the Merkle leaf input); `object_canonical` stores the full envelope so the
content address is exactly re-derivable. Append-only: rows are never updated or
deleted (retention is archive-to-S3, never destruction — §6.2g).
