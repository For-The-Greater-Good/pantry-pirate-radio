...
class AggregatedMapDataExporter:
    def __init__(
        ...
        grouping_radius_meters: Optional[int] = None,
    ):
        ...
        if grouping_radius_meters is not None:
            ...
            self.grouping_radius_meters = int(
                os.getenv("MAP_GROUPING_RADIUS_METERS", "150")
            )
    ...
    def export(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Aggregated map data export completed in {elapsed:.2f} seconds"
            )
        ...
        for _, group in location_groups.items():
            ...
            for loc in group:
                ...
                if scraper_id and scraper_id not in scrapers_seen:
                    ...
                    if (
                        loc.get("opens_at")
                        or loc.get("closes_at")
                        or loc.get("schedule_description")
                    ):
                        schedule = {
                            "opens_at": (
                                str(loc["opens_at"]) if loc.get("opens_at") else None
                            ),
                            "closes_at": (
                                str(loc["closes_at"]) if loc.get("closes_at") else None
                            ),
                    ...
                    source_data = {
                        ...
                        "first_seen": (
                            loc["first_seen"].isoformat()
                            if loc.get("first_seen")
                            else None
                        ),
                        "last_updated": (
                            loc["last_updated"].isoformat()
            ...
            aggregated_location = {
                ...
                "name": primary.get("location_name")
                or primary.get("org_name")
    ...
    def _generate_output_files(self, locations: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            multi_source_locations = sum(
                1 for loc in locations if loc["source_count"] > 1
            )
            ...
            logger.info(
                f"Writing {len(locations)} aggregated locations to {output_file}"
            )
