...
class MapDataExporter:
    ...
    def export(self) -> bool:
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                logger.info(f"Found {total_count} canonical locations with valid coordinates")
    ...
    def _generate_state_files_parallel(self, locations: List[Dict[str, Any]], 
                                      base_metadata: Dict, data_dir: Path):
        ...
        def normalize_state(state_str: str) -> str:
            ...
            multi_word_prefixes = {'NEW', 'NORTH', 'SOUTH', 'WEST', 'RHODE', 'DISTRICT', 
                                  'AMERICAN', 'NORTHERN', 'VIRGIN', 'US'}
        ...
        locations_by_state = {}
        invalid_states = set()
        state_mapping_stats = {}  # Track how states were mapped
        ...
        for location in locations:
            ...
            if original_state and normalized_state != original_state:
                if normalized_state == "UNKNOWN" and len(original_state) < 100:
                    ...
                    state_mapping_stats[mapping_key] = state_mapping_stats.get(mapping_key, 0) + 1
        ...
        if invalid_states:
            logger.warning(f"Found {len(invalid_states)} unique unmappable state values")
        
        if state_mapping_stats:
            logger.info(f"Successfully mapped {len(state_mapping_stats)} non-standard state values")
            # Log a few examples
