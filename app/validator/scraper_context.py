...
def enhance_address_with_context(
    address: Dict[str, Any], scraper_id: Optional[str] = None
) -> Dict[str, Any]:
...
def format_address_for_geocoding(
    ...
    if scraper_id and formatted != ", ".join(
        filter(
            None,
            [
                ...
                " ".join(
                    filter(
                        None,
                        [address.get("state_province"), address.get("postal_code")],
                    )
                ),
            ],
        )
...
def is_address_in_scraper_region(
    latitude: float, longitude: float, scraper_id: str
) -> bool:
