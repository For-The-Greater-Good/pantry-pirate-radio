...
from app.scraper.second_harvest_food_bank_of_northwest_north_carolina_nc_scraper import (
    SecondHarvestFoodBankOfNorthwestNorthCarolinaNCScraper,
)
...
async def test_download_html_success(
    scraper: SecondHarvestFoodBankOfNorthwestNorthCarolinaNCScraper,
...
async def test_scrape_html_flow(
    ...
    with patch("requests.get") as mock_get:
        ...
        assert (
            job["source"] == "second_harvest_food_bank_of_northwest_north_carolina_nc"
        )
        assert (
            job["food_bank"] == "Second Harvest Food Bank of Northwest North Carolina"
        )
...
def test_scraper_initialization():
    ...
    assert (
        scraper1.scraper_id == "second_harvest_food_bank_of_northwest_north_carolina_nc"
    )
    assert scraper1.test_mode is False

    # Test with custom ID
    scraper2 = SecondHarvestFoodBankOfNorthwestNorthCarolinaNCScraper(
        scraper_id="custom_id"
    )
