...
class TestLoadScraperClass:
    ...
    def test_load_single_scraper_class(self, mock_import: Mock) -> None:
        ...
        with patch("builtins.dir", return_value=["SampleScraper", "some_function", "_private_class"]):
    ...
    def test_load_multiple_scraper_classes_fallback(self, mock_logger: Mock, mock_import: Mock) -> None:
...
class TestListAvailableScrapers:
    ...
    def test_list_scrapers_from_directory(self, mock_scraper_dir: Path) -> None:
        ...
        with patch("app.scraper.__main__.__file__", str(mock_scraper_dir / "__main__.py")):
            scrapers = list_available_scrapers()
        
        expected = ["another", "not_a", "sample", "test"]  # sorted order, all *_scraper.py files
...
class TestRunScraperParallel:
    ...
    async def test_run_scraper_success(self) -> None:
        ...
        with patch("app.scraper.__main__.load_scraper_class", return_value=MockScraper) as mock_load:
    ...
    async def test_run_scraper_failure(self) -> None:
        ...
        with patch("app.scraper.__main__.load_scraper_class", return_value=FailingScraper) as mock_load:
            with patch("app.scraper.__main__.logger") as mock_logger:
    ...
    async def test_run_scraper_load_error(self) -> None:
        ...
        with patch("app.scraper.__main__.load_scraper_class", side_effect=ImportError("Module not found")):
            with patch("app.scraper.__main__.logger") as mock_logger:
...
class TestRunAllScrapersParallel:
    ...
    async def test_run_all_scrapers_success(self) -> None:
        ...
        with patch("app.scraper.__main__.list_available_scrapers", return_value=mock_scrapers):
...
class TestMain:
    ...
    async def test_main_single_scraper_runtime_error(self) -> None:
        ...
        with patch("sys.argv", ["__main__.py", "failing_scraper"]):
            with patch("app.scraper.__main__.load_scraper_class", return_value=FailingScraper):
                with patch("app.scraper.__main__.logger") as mock_logger:
                    with patch("sys.exit") as mock_exit:
                        await main()
    ...
    async def test_main_all_scrapers_sequential_with_failure(self) -> None:
        ...
        with patch("sys.argv", ["__main__.py", "--all"]):
            with patch("app.scraper.__main__.list_available_scrapers", return_value=mock_scrapers):
                with patch("app.scraper.__main__.load_scraper_class", side_effect=mock_load_scraper):
                    with patch("app.scraper.__main__.logger") as mock_logger:
                        await main()
    ...
    async def test_main_all_scrapers_parallel(self, capsys) -> None:
        ...
        with patch("sys.argv", ["__main__.py", "--all", "--parallel", "--max-workers", "2"]):
            with patch("app.scraper.__main__.run_all_scrapers_parallel", return_value=mock_results) as mock_run:
                with patch("app.scraper.__main__.logger") as mock_logger:
                    with patch("sys.exit") as mock_exit:
                        await main()
        
        mock_run.assert_called_once_with(2)
        mock_logger.info.assert_any_call("Running all scrapers in parallel with max 2 workers")
    ...
    async def test_main_all_scrapers_parallel_success(self, capsys) -> None:
        ...
        with patch("sys.argv", ["__main__.py", "--all", "--parallel"]):
            with patch("app.scraper.__main__.run_all_scrapers_parallel", return_value=mock_results):
                with patch("sys.exit") as mock_exit:
                    await main()
    ...
    async def test_main_argument_parsing(self) -> None:
        ...
        with patch("sys.argv", ["__main__.py", "test_scraper", "--all"]):
            with patch("app.scraper.__main__.load_scraper_class", return_value=MockScraper):
...
class TestErrorHandling:
    ...
    async def test_value_error_handling(self) -> None:
        ...
        with patch("sys.argv", ["__main__.py", "test_scraper"]):
            with patch("app.scraper.__main__.load_scraper_class", side_effect=ValueError("Invalid scraper")):
                with patch("app.scraper.__main__.logger") as mock_logger:
                    with patch("sys.exit") as mock_exit:
                        await main()
    ...
    async def test_generic_exception_handling(self) -> None:
        ...
        with patch("sys.argv", ["__main__.py", "test_scraper"]):
            with patch("app.scraper.__main__.load_scraper_class", return_value=FailingMockScraper):
                with patch("app.scraper.__main__.logger") as mock_logger:
                    with patch("sys.exit") as mock_exit:
                        await main()
...
class TestCoverageEdgeCases:
    ...
    def test_load_scraper_underscore_normalization(self) -> None:
        ...
        with patch("app.scraper.__main__.importlib.import_module", return_value=mock_module):
    ...
    def test_list_available_scrapers_file_filtering(self, tmp_path: Path) -> None:
        ...
        expected = ["", "also_valid", "not", "valid"]  # sorted - includes all *_scraper.py files
        assert scrapers == expected
