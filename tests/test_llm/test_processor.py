...
def test_handle_claude_errors_with_non_claude_exception(
    ...
    """Test handle_claude_errors with a non-Claude exception re-raises it."""
    from app.llm.queue.processor import handle_claude_errors

    # Create a non-Claude exception
    generic_error = ValueError("Not a Claude error")

    # Should re-raise as RuntimeError when not in exception context
    with pytest.raises(RuntimeError):
        handle_claude_errors(generic_error, sample_job)


def test_handle_claude_errors_import_error(
    sample_job: LLMJob,
) -> None:
    """Test handle_claude_errors when Claude provider is not available."""
    from app.llm.queue.processor import handle_claude_errors
    ...
    with patch.dict("sys.modules", {"app.llm.providers.claude": None}):
        # Should re-raise the original exception when import fails
        with pytest.raises(RuntimeError, match="Some error"):
            handle_claude_errors(error, sample_job)
