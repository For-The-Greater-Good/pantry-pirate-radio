...
class TestValidatorIntegration:
    ...
    def test_full_pipeline_with_validator(self, db_session, mock_redis):
        ...
        # Create LLM job with valid SHA-256 hash
        job = LLMJob(
            ...
            provider_config={},
            metadata={
                "content_hash": "a" * 64,  # Valid 64-character hex hash
        ...
        with patch("app.core.config.settings.VALIDATOR_ENABLED", True):
            ...
            with patch(
                ...
                assert validation_result["job_id"] == job_result.job_id
                assert validation_result["status"] == "passed_validation"
                assert "data" in validation_result
                # Verify the data was parsed from the LLM response
                assert "organization" in validation_result["data"]
                assert "locations" in validation_result["data"]
    ...
    def test_validator_worker_processing(self, mock_redis):
        ...
        # Create worker - it doesn't take a redis argument
    ...
    def test_validator_database_integration(self, db_session):
        ...
        # Create test data with unique name to avoid conflicts
        import uuid

        unique_suffix = str(uuid.uuid4())[:8]
        org = Organization(
            name=f"Validator Test Org {unique_suffix}", description="Test"
        )
        db_session.add(org)
        db_session.commit()

        location = Location(
            name=f"Validator Test Location {unique_suffix}",
            latitude=40.7128,
            longitude=-74.0060,
    ...
    def test_validator_disabled_integration(self):
        ...
        with patch("app.core.config.settings.VALIDATOR_ENABLED", False):
            ...
            assert "validator" not in pipeline["stages"]
