...
class TestValidationServiceBase:
    ...
    def test_validation_service_initialization(self, db_session: Session):
        ...
        assert service.logger.name == "app.validator.base"
    ...
    def test_validation_service_with_job_result(self, db_session: Session, sample_hsds_job):
    ...
    def test_validation_service_without_logging(self, db_session: Session, caplog):
        ...
        with caplog.at_level(logging.INFO):
            ...
            assert "Validation service passing through data unchanged" not in caplog.text

    def test_validation_service_configuration(self, db_session: Session, validator_config):
    ...
    def test_validation_service_disabled(self, db_session: Session, disabled_validator_config):
        """Test that ValidationService can be disabled via configuration."""
        service = ValidationService(
            db=db_session,
            config=disabled_validator_config
        )
