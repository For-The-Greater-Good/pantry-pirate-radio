...
class TestBackwardCompatibility:
        ...
        with patch("app.reconciler.job_processor.create_engine"):
            with patch("app.reconciler.job_processor.sessionmaker"):
                with patch("app.reconciler.job_processor.JobProcessor") as MockProcessor:
                    ...
                    mock_instance.process_job_result.return_value = {"status": "success"}
    ...
    def test_database_fields_optional(self, db_session):
        ...
        assert location.confidence_score is None  # Should default to None
        assert location.validation_notes is None
        assert location.validation_status is None
        assert location.geocoding_source is None

    def test_api_endpoints_unchanged(self):
        """Test that API endpoints work unchanged."""
        from app.api.v1.endpoints import organizations, locations, services
