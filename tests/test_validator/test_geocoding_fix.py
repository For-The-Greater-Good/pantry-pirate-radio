...
class TestScraperContext:
    ...
    def test_food_bank_nyc_context(self):
        ...
        enhanced = enhance_address_with_context(address, "food_bank_for_new_york_city_ny")
        assert enhanced["state_province"] == "NY"
        assert enhanced["city"] == "New York"
        
        formatted = format_address_for_geocoding(address, "food_bank_for_new_york_city_ny")
...
class TestGeocodingEnricher:
    ...
    def test_all_providers_tried(self, mock_service_class):
        ...
        census_calls = [
            call for call in mock_service.geocode_with_provider.call_args_list
            if 'census' in str(call)
        ]
