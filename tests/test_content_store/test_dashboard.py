...
class TestDashboardApp:
    ...
    @patch.dict("os.environ", {"DASHBOARD_HOST": "0.0.0.0", "DASHBOARD_PORT": "8080"})
...
class TestDashboardRoutes:
    ...
    @patch("app.content_store.dashboard.Path")
    def test_api_stats_success(self, mock_path, mock_connect, mock_redis, mock_get_store, client):
        ...
        with patch.object(Job, 'fetch', return_value=mock_job):
            ...
            mock_db_path.__truediv__.return_value.__truediv__.return_value = "/data-repo/content_store/index.db"
    ...
    @patch("app.content_store.dashboard.get_redis_connection")
    @patch("app.content_store.dashboard.sqlite3.connect")
    def test_api_stats_with_job_fetch_exception(self, mock_connect, mock_redis, mock_get_store, client):
    ...
    @patch("app.content_store.dashboard.get_redis_connection")
    @patch("app.content_store.dashboard.sqlite3.connect")
    def test_api_stats_with_content_read_exception(self, mock_connect, mock_redis, mock_get_store, client):
        ...
        mock_cursor = Mock()
        mock_cursor.__iter__ = Mock(return_value=iter([
            ('hash123', 'completed', None, '2023-12-01 10:00:00')
        ]))
        mock_conn.execute.return_value = mock_cursor
        mock_connect.return_value = mock_conn
...
class TestDashboardIntegration:
    ...
    @patch("app.content_store.dashboard.get_content_store")
    @patch("app.content_store.dashboard.get_redis_connection") 
    def test_api_stats_integration(self, mock_redis, mock_get_store, client, temp_db_path):
        ...
        with patch("app.content_store.dashboard.Path") as mock_path:
            mock_path.return_value.__truediv__.return_value.__truediv__.return_value = temp_db_path
