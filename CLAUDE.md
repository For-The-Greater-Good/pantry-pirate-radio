...
- **Archive tiering + retention prune (closes #522)** — `app/federation/retention.py`. The verifiable log recomputes every checkpoint root + proof on demand from the full `preimage_canonical` leaf bytes (no persisted Merkle frontier), so trimming a leaf would break `signed_checkpoint` and every proof spanning it. "Never destroy" (§6.2g) is therefore **archive-then-trim with read-back**: `prune_to_horizon` archives each over-SLA leaf's exact signed bytes to a never-expiring tier (write-ahead — archive durably BEFORE the live DELETE; an archive-put failure HALTS, no leaf loss) then trims the live Postgres window contiguously; `log.leaf_data` reads below-floor leaves back from the archive, so root@N + inclusion/consistency proofs across the trim boundary stay valid forever. `ArchiveBackend` = `LocalFsArchiveBackend` (Docker, atomic write) or `S3ArchiveBackend` (AWS, dedicated **no-lifecycle** bucket). `retention_horizon_sequence` (= `live_window_floor`) is exposed UNSIGNED on `/checkpoint` + a `Federation-Retention-Horizon` header on `/state.txt` (renamed from `X-Federation-Retention-Horizon` in Slice W per RFC 6648; never in the signed C2SP note body). Dual-env (XV): `./bouy federation prune` (refuses without an archive tier) and an EventBridge-scheduled prune Lambda (`infra/stacks/federation_stack.py`, prod-only-enabled) — both call the same `prune_to_horizon`. XIV: a prune-Lambda Errors alarm (`monitoring_alarms.py`) → `pantry-pirate-radio-alerts-{env}`. Grep CloudWatch for `federation_archive_tiered` / `federation_archive_failed`. Below-floor `/export` still 410s (the cold-start S3/SQLite **snapshot** serving — bulk fresh-peer bootstrap — is the documented deferred follow-on; the archive WRITE + internal read-back ship here).

**P2 status (in progress):**
- **Slice W — W3C `eddsa-jcs-2022` DataIntegrityProof adopted (wire-freeze D1).** The envelope proof is now the off-the-shelf W3C Data Integrity `eddsa-jcs-2022` cryptosuite (`app/federation/di_proof.py`, anchored by the vendored W3C KAT `tests/test_federation/vendor/w3c_eddsa_jcs_2022/` + `test_eddsa_jcs_2022_kat.py`, which reproduces the published `proofValue` byte-for-byte) — `proof.type="DataIntegrityProof"`, `cryptosuite="eddsa-jcs-2022"`, multibase-base58btc `proofValue`, `proofPurpose="assertionMethod"`, `created` defaulting to `published`, signed over the DI document (envelope minus `proof`, `id` included). This **replaces** the bespoke `ed25519-jcs-2026` proof. The content address is proof-format-independent and stayed byte-identical (`test_vector_invariance.py` pins the proof-independent vectors). Also in Slice W: export/checkpoint headers renamed `X-Federation-*` → `Federation-*` (RFC 6648); `verify_note` now rejects ASCII control chars below U+0020 except newline (C2SP signed-note accept-set); an I-JSON integer bound (RFC 7493 §2.2, `|v| <= 2^53-1`) enforced at the envelope boundary (so `canonical.py` stays a general RFC-8785 serializer). Conformance vectors `envelope_proof.json` + `envelope_assembly.json` re-baked at suite `VERSION 0.3.0`.
